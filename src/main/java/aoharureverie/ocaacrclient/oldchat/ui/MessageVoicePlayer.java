...
public class MessageVoicePlayer {
    private static final String TAG = "MessageVoicePlayer";
    ...
    public MessageVoicePlayer(Context context, PlaybackListener listener) {
    ...
    public boolean isLoading(Message msg) {
        synchronized (this) {
            return isTargetLocked(msg) && (state == STATE_DOWNLOADING || state == STATE_PREPARING);
        }
    }

    public boolean isPlaying(Message msg) {
        synchronized (this) {
            return isTargetLocked(msg) && state == STATE_PLAYING;
        }
    }

    public String getDurationLabel(Message msg, int seconds) {
        synchronized (this) {
            if (!isTargetLocked(msg)) {
                return seconds + "\"";
            }
        ...
        try {
            ...
            player.setOnPreparedListener(new MediaPlayer.OnPreparedListener() {
                ...
                public void onPrepared(MediaPlayer mp) {
                    synchronized (MessageVoicePlayer.this) {
    ...
    private void setPlayingMessageLocked(Message msg) {
        playingId = msg.id;
        playingCreatedAt = msg.created_at;
    }

    private boolean isTargetLocked(Message msg) {
        if (msg == null) {
            return false;
        }
